...
class JaxResNet(Tomographer):
    ...
    def train (self, training_data, training_z):
        ...
        class ResNetBlock(nn.Module):
            def apply(self, x, filters, conv, norm, act, strides):
                residual = x
                out = conv(x, features=filters, kernel_size=(1,))
                out = norm(out)
                out = act(out)
                out = conv(out, features=filters, kernel_size=(3,), strides=(1,))
                out = norm(out)
                out = act(out)
                out = conv(out, features=filters * 4, kernel_size=(1,))
                out = norm(out, scale_init=nn.initializers.zeros)

                if residual.shape != out.shape:
                    residual = conv(residual, features=filters * 4, kernel_size=(1,), strides=(1,))
                    residual = norm(residual)

                return act(residual + out)
