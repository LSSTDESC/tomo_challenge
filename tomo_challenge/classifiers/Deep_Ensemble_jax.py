...
class ENSEMBLE(Tomographer):
    ...
    def __init__ (self, bands, options):
        ...
        self.bands = bands
        self.opt = options
        self.CLASSIFIERS = ['resnet_jax', 'autolstm_jax']
    ...
    def fit_classifier(self, classifier_name, x_train, z_train, nb_classes, output_directory,  load_weights=False):

        input_shape = x_train.shape[1:]

        if classifier_name == 'resnet_jax':
            from tomo_challenge.classifiers import resnet_jax
            # resnet = resnet_jax.Resnet(nb_classes)
            resnet = resnet_jax.Resnet(n_bins=nb_classes)
            _, model = resnet_jax.train(resnet,x_train.shape[1], nb_classes, x_train, z_train)
            self.model_res = model
        else:
            from tomo_challenge.classifiers import autolstm_jax
            _, model = autolstm_jax.train(x_train.shape[1], nb_classes, x_train, z_train)
            self.model_auto = model
    ...
    def create_classifier(self, classifier_name, input_shape, nb_classes, output_directory, verbose=True,
        ...
        # if classifier_name == 'resnet_jax':
        #     from tomo_challenge.classifiers import resnet_jax
        #     return 0#resnet_jax.Resnet(nb_classes)
        # if classifier_name == 'autolstm_jax':
        #     from tomo_challenge.classifiers import autolstm_jax
        #     return autolstm_jax.AutoLSTM(nb_classes)
    ...
    def train (self, training_data, training_z):
        ...
        for classifier_name in self.CLASSIFIERS:
            ...
            self.fit_classifier(classifier_name=classifier_name, x_train=x_train, z_train=training_z,
    ...
    def apply (self, data):
        ...
        from tomo_challenge.classifiers import nne
        ...
        preds = ensemble.predict(data, self.model_res, self.model_auto)
