...
\begin{abstract}
This paper presents the results of the LSST-DESC 3x2pt tomography challenge, which served as a first step toward optimizing the tomographic binning strategy for the main LSST 3x2pt cosmology analysis. 
The task of choosing an optimal tomographic binning scheme for a photometric survey is made particularly delicate in the context of a metacalibrated lensing catalog, as only the photometry from the bands included in the metacalibration process (traditionally, {\em riz} and potentially {\em g}) can be used in sample definition.
...
% AIM: Suggestion: Use a macro for DESC because it shows up with and without expansion all over
...
% AIM: The next two paragraphs should present tomographic binning as an experimental design choice up front. 

Most lensing and 3x2pt analyses have been performed \emph{tomographically}, 
binning galaxies by redshift.
% AIM: I get what this paragraph is saying, but it's worded ambiguously and can read as almost its exact opposite. I suggest rewording to specify more of the instances of "this," "that," and "it."
...
% AIM: typo in the next sentence {potentially the ease}
...
% AIM: Move the following sentence two paragraphs up to where tomography is introduced
...
% AIM: Above sentence should cite other strategies as well.

% AIM: insert comma {paper we}
% AIM: messaging -- establish scope with the basic info now in the abstract
...
different bins.
% AIM: We must explicitly state the assumptions that all methods were expected to be ML/training-based and that this is framed as a classification problem, and explain how that fits with the paradigm of how people think about tomography, which is traditionally framed as a very different redshift regression problem.
...
% AIM: Was it actually advertised in Rubin beyond DESC at all?
...
% AIM: "tested and ran" --> "developed and tested" 
% AIM: remove comma {locally, but}
% AIM: Final tests used to generate results on secret test set were conducted on NERSC, no?
...
% AIM: Don't specify NERSC up here before it's defined in Sec. 3.4.
...
\begin{figure}[htbp]
	...
	% AIM: specify /redshift/ number density
...
% AIM: Rest of paper uses past tense so "run" --> "ran"
% AIM: Train/Validate/Test set splitting should be described in post-processing rather than preamble, and assumption of ML/training-based approach should be stated explicitly in preamble to entire section.
...
% AIM: We don't have to justify the second dataset unless we provide a reason why the first was insufficient. . . the presentation here needs some reworking.
...
% AIM: typo {Each of these data set}
...
% AIM: Say it was also used for DESC DC1 studies, cite e.g. PZ DC1 paper
...
% AIM: Put the info from CosmoDC2/Buzzard pertaining to status after post-processing steps here instead.
...
% AIM: specify /redshift/ number density and provide n(z) symbol because it is used without introduction later
...
% AIM: If submitting to AAS journal, colour --> color; if submitting to MNRAS, catalog --> catalogue

\subsection{Metrics}
\label{sec:metrics}
% AIM: Change subsubsection titles to use the shorthand names for the metrics as they are referenced throughout the rest of the paper (SNR, w_{0}-w_{a} FOM, \Omega_{c}-\sigma_{8} FOM).
% AIM: This should state that the three metrics are evaluated for clustering, lensing, and 3x2pt, so there are really nine metrics, not two!
...
% AIM: Sounds dumb, but readers will appreciate a clear statement that higher is better for these metrics.

\subsubsection{Metric 1: Sigal-to-noise}
% AIM: Suggestion: merge Appendix A with this section because this really doesn't stand by itself
...
% AIM: The title of this subsubsection should reflect that there are two FOM metrics.
...
% AIM: We must acknowledge GitHub as a platform that enabled this challenge.
% AIM: Suggestion to specify "subclass" and "superclass" up front for extra-thorough clarity.
...
% AIM: Include the code names for these dummy/control methods up front
Two control methods were used to test the challenge machinery and metrics.
% AIM: Explicitly put this into the context of a controlled experiment.
% AIM: Make a new paragraph for each control method.
...
% AIM: This should be a subsection of Challenge Design section above since it's really just a table of the submitted methods directing readers to the appendix for any detail.

% AIM: typo: "Twenty five" --> "Twenty-five"
...
% AIM: These should be a pagewidth table, and we should use macros so the method names display consistently throughout paper.
...
% AIM: Convert the first two subsections to the preamble of this section.
% AIM: Preamble should include the tl;dr of "winners."

\subsection{Result types}

% AIM: Move this table to the appendix.
\begin{table*}[]
...
\subsection{Overview}
% AIM: Suggestion: Convert this subsection to a bulleted list to give it some structure harking to the detailed subsections to come. The short versions are long enough to come off as unsupported statements rather than previews of what will be demonstrated in subsequent subsections. 
...
% AIM: This is the first mention of this as a classification problem. We should mention this up in experimental design along with the assumption of ML/training-based methods, but also change "clasified" [SIC] to "assigned" for overall consistency (and maybe "that entrants clasified" --> "entrants to classify," and maybe reword post-semicolon part as "aside from the impact of reduced final sample size on the metrics, no penalty was applied for discarding galaxies entirely").
% AIM: Also, the discussion of non-assignment should be its own subsection.
...
% AIM: Split paragraph here to distinguish topics of overall challenge winner and conclusions about band information limitation.
% AIM: Put the caveat at the beginning of the following sentence rather than the end. Also, this claim is not well-supported even by Fig. 3. . . 
...
% AIM: This should not be an appendix and should instead be part of the Results & Discussion section. They should not be presented as "mistakes" -- they are part of the experimental design and constitute caveats, which are simply facts, not subjective judgements.
...
\section{Full 9-bin results table} \label{app:tables}
